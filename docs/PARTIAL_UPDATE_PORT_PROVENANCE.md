...
| `src/index.ts` broadcast filter | Client-metadata filtered delivery | Behavioral reimplementation | Channel / `subscribe_conversation_sync` / turn subscribers | Partial — Channel for private kinds; Sync/Conflict prefer conversation-scoped subscribers | Residual global Sync only when no scoped subscriber; queue-drain Action/Error/Operation dropped (no progress UI) |
