...
| `src/index.ts` withQueueMutation | Serialized queue mutations, one active | Substantially adapted | `runtime_v2/queue.rs` + `ConversationQueue.tsx` + `agent-queue-changed` | Partial — backend + event UI + 20s reconcile | Conversation-scoped queue Channel (global emit + client filter remains P1) |
