...
describe("Journey 8 — grant revoke", () => {
  it("revokes the seeded remembered grant from App permissions", async () => {
    requireExistingSeed("Journey 8");

    await waitForAppReady();
    await denyPendingApprovalIfPresent();
    await openSettings();
    await scrollSettingsToHeading("App permissions");
    ...
    await browser.waitUntil(
      ...
      {
        timeout: 20_000,
        timeoutMsg: "Seeded remembered grant never appeared in App permissions",
      },
    );
